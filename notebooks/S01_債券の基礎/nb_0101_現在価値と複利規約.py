...
# ### 割引係数とゼロレートの関係
#
# **ゼロクーポン債とは。** 途中の利払い（クーポン）が一切なく、満期に額面だけを1回受け取る最もシンプルな債券です（割引債とも呼びます）。今は額面より安く買い、満期に額面を受け取り、その差額が実質的な利息になります。たとえば1年後に100円を受け取る債券が今97.09円で買えるなら、$97.09 \times (1+r) = 100$ から利回り $r = 3\%$ が逆算できます。キャッシュフローが「満期の1回だけ」なので、その価格はそのまま『その満期までの純粋な割引率』を表し、そこから逆算した利回りがゼロレートです。逆に、途中でクーポンを払う普通の債券は、満期の違う複数のゼロクーポン債を束ねたものと見なせます。
#
...
# %% [markdown]
# **数値例**：割引係数 $DF(5)=0.90$（$t=5$ 年）を連続複利ゼロレートへ逆算すると $r=-\dfrac{\ln 0.90}{5}=0.021072=2.1072\%$ です。

# %% [markdown]
# **数値例**：連続複利 $r=3\%$、$t=5$ 年なら割引係数は $DF(5)=e^{-0.03\times 5}=e^{-0.15}=0.860708$ です。額面 $C=100$ のキャッシュフローの現在価値は $PV=100\times 0.860708=86.0708$ となります。

# %% [markdown]
# **数値例**：半年複利 $r_m=5\%$（$m=2$）を連続複利に直すと
# $r_c = 2\ln(1+0.05/2) = 2\ln(1.025) = 0.049385 = 4.9385\%$。
# 逆に戻すと $r_m = 2(e^{0.049385/2}-1) = 0.05$ で一致します。
#
