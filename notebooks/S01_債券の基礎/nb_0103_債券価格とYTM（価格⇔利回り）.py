...
# %% [markdown]
# **数値例**：クーポン $C_{\%}=2$、価格 $P=98$、残存 $T=5$ 年の単利最終利回りは $y_{\text{simple}}=\dfrac{2+(100-98)/5}{98}=\dfrac{2.4}{98}=0.024490=2.4490\%$ です。ディスカウント債なので、複利YTMよりやや高めに出ます。
# %% [markdown]
# **数値例**：$c=4\%$・半年払い（$f=2$）・額面 $F=100$・$y=5\%$、残り $n=4$ 回なら、1回のクーポンは $C=100\times 0.04/2=2$ で、価格は $P=\sum_{k=1}^{3}\dfrac{2}{1.025^{k}}+\dfrac{2+100}{1.025^{4}}=98.1190$ です。$c<y$ なのでディスカウント債になります。
# %% [markdown]
# ## スクラッチ実装
#
