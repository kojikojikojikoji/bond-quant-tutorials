...
# | 用語 | 英語 | 一行定義 |
# |---|---|---|
# | ブートストラップ | bootstrap | パー利回り等から割引係数を逐次に剥ぎ取る前進代入の手続き |
# | ゼロカーブ | zero curve | 各年限の単一支払いに対するゼロレートを並べたカーブ |
# | フォワードレート | forward rate | いま約定し将来区間に適用される金利。ゼロの限界に対応 |
# | パーレート | par rate | 標準クーポン債が額面で取引されるときのクーポン率 |
# | 剥ぎ取り | strip | 短期から順に割引係数を確定させていくブートストラップの逐次操作 |
