...
# %% [markdown]
# **数値例**：対象（10年、$\mathrm{DV01}=0.0804$／額面100）をヘッジ商品（2年、$\mathrm{DV01}=0.0193$／額面100）でヘッジするなら $h=-\dfrac{0.0804}{0.0193}=-4.17$ です。10年債を額面100ロングするたびに、2年債を約 417 額面ショートすれば平行シフトの一次損益が打ち消せます。
# %% [markdown]
# **数値例**：クーポン4.35%・10年・平価発行（$P=100$、$D_{\mathrm{mod}}=8.0393$）の債券では $\mathrm{DV01}=8.0393\times 100\times 10^{-4}=0.0804$（額面100あたり）です。額面 1,000万を持てば $\mathrm{DV01}=0.0804\times\frac{10{,}000{,}000}{100}=8{,}039$ 円/bp となり、金額の感応度に揃います。
# %% [markdown]
# ## スクラッチ実装
#
