...
# %% [markdown]
# **数値例**：信頼水準 $\alpha=0.99$（$p=1-\alpha=0.01$）で $n=250$ 営業日をバックテストするなら、期待例外数は $pn=0.01\times250=2.5$ 件です。実際の例外数がこの $2.5$ 件からどれだけ離れているかを Kupiec 検定で尤度比により判定します。
# %% [markdown]
# **数値例**：各資産が確率 $p=0.03$ で損失 $L=100$、確率 $0.97$ で利益 $c=1$ を出すとします（$\alpha=0.95$）。単独では $p<1-\alpha=0.05$ なので $(1-\alpha)$ 分位点が利益側に落ち $\mathrm{VaR}(A)=\mathrm{VaR}(B)=-1$。独立2資産の合算では損失確率が $1-0.97^2=0.0591>0.05$ に上がり $\mathrm{VaR}(A+B)=99$ となります。$99>-1-1=-2$ で $\mathrm{VaR}(A+B)>\mathrm{VaR}(A)+\mathrm{VaR}(B)$、劣加法性が破れます。
# %% [markdown]
# **数値例**：日次損益が $X\sim\mathcal{N}(\mu=0.02,\ \sigma=1.3)$、$\alpha=0.99$ のとき、$z_{0.01}=\Phi^{-1}(0.01)=-2.326$ を使って $\mathrm{VaR}_{0.99}=-(0.02+1.3\times(-2.326))=3.004$、$\mathrm{ES}_{0.99}=-\big(0.02-1.3\times\tfrac{\phi(z_{0.01})}{0.01}\big)=3.445$ です。ES が VaR より裾の分だけ大きくなります。
# %% [markdown]
# ## スクラッチ実装
#
