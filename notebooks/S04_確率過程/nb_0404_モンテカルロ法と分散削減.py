...
# %% [markdown]
# **数値例**（$1/\sqrt{n}$ の遅さ）：標準誤差は $\operatorname{SE}=\sigma/\sqrt{n}$ なので、パス数を $n$ から $100n$ へ100倍にして初めて $\operatorname{SE}$ が $1/\sqrt{100}=1/10$ に縮みます。逆に $\operatorname{SE}$ を半分にするだけでも $4$ 倍のパスが要ります。
...
# %% [markdown]
# **数値例**（制御変量の削減率）：目標と制御の相関が $\rho_{XY}=0.95$ なら分散は $1-\rho_{XY}^2=1-0.9025=0.0975$ 倍（約10分の1）に下がります。$\rho_{XY}=0.99$ まで上げれば $1-0.9801=0.0199$ 倍（約50分の1）で、相関を1に近づけるほど効きます。

# %% [markdown]
# **数値例**（対照変量の効き）：対の相関が $\rho=-0.5$ なら $\operatorname{Var}(\bar X)=\tfrac{\sigma^2}{2}(1+\rho)=\tfrac{\sigma^2}{2}\cdot0.5=\sigma^2/4$ で、独立2標本の $\sigma^2/2$ の半分です。一方 $\rho=+0.5$ だと $\tfrac{\sigma^2}{2}\cdot1.5=0.75\sigma^2$ となり、かえって悪化します。
