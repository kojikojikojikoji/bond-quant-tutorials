...
# #### 数値例
#
# 以下は $a=0.3,\ b=0.05,\ \sigma=0.02,\ r_t=0.03$、残存 $\tau=5$ 年での具体値です。
#
# **数値例**：$B(\tau)=\dfrac{1-e^{-0.3\times 5}}{0.3}=2.590$ です。
#
# **数値例**：$A(\tau)=0.8892$ なので、割引債は $P=A\,e^{-B r_t}=0.8892\times e^{-2.590\times 0.03}=0.8228$、ゼロレートは $y=-\ln P/\tau=3.90\%$ となります。
#
# **数値例**：定常分布の標準偏差は $\sqrt{\sigma^2/(2a)}=\sqrt{0.02^2/(2\times 0.3)}=2.58\%$ で、平均回帰により発散せず有限に留まります。
# %% [markdown]
# ## スクラッチ実装
#
...
# | 用語 | 英語 | 一行定義 |
# |---|---|---|
# | [平均回帰](../../glossary/05_rate_models.md#mean-reversion) | mean reversion | 変数を長期水準へ引き戻す力。金利モデルの基本性質 |
# | [アフィンモデル](../../glossary/05_rate_models.md#affine-model) | affine model | ZCB 価格が金利の指数アフィン形になる短期金利モデル |
# | [ゼロクーポン債（ZCB）](../../glossary/05_rate_models.md#zcb-zero-coupon-bond) | zero-coupon bond | 満期に額面のみを払う債券。割引係数そのもの |
# | [リスクの市場価格](../../glossary/05_rate_models.md#market-price-of-risk) | market price of risk | 実測度とリスク中立測度をつなぐ量。長期水準を付け替える |
