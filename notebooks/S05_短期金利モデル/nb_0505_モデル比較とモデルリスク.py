...
# ## 実務での位置づけ（ファンドはどう稼ぐか）
#
# このモデルリスクの notebook は、金利デスクの意思決定そのものを扱います。Vasicek・CIR・Hull-White のどれで同じコーラブル債やスワップションを評価するかで、価格と Greeks が食い違います。デスクは分布特性（正規か非心カイ二乗か）、負金利の許容、初期カーブ整合という比較軸を踏まえてモデルを選び、その差がマーケットメイクのクォートやヘッジ比率に反映されます。「どのモデルを選ぶか」という選択自体が損益とリスクの源泉になる、というのが本 notebook で数値化する核心で、`docs/債券ファンドの業務.md` が言うマクロ・相対価値・マーケットメイクのいずれの稼ぎ方にも通底します。
#
# モデル選択が収益に効くのは、価格差が実際の売買と担保・損益計算に乗るからです。ある商品を割安と判断して買うか、ヘッジのベガ・デルタをいくらに置くかは、採用モデルの Greeks で決まります。モデルを変えれば評価損益もヘッジ量も動くため、同じポジションでも損益の出方が変わります。低金利・マイナス金利局面で「負金利を許すか否か」が価格を大きく分けるのは、フロア性を持つ商品の裾の価値がモデルの分布仮定に直結するためで、モデル選択の誤りが体系的なミスプライスとして積み上がります。
#
# 本 notebook はモデルバリデーション（FRB SR 11-7）の実務そのものを体験する構成です。概念的健全性（比較軸・分布特性の妥当性）、継続的モニタリング（パラメータを振る感応度分析）、アウトカム分析（QuantLib 検証・3モデル相互比較・カーブ再現誤差）の3本柱に沿って、独立検証とベンチマークモデル比較を回し、Hull-White の承認レポートを書きます。さらに1ファクターの限界（全テナーが1つの因子で動く＝カーブの傾き変化を表せない）を踏まえ、G2++・HJM・LMM といった多因子拡張へ進む判断軸を俯瞰します。この検証・承認の枠組みが、モデル要因の損失を未然に抑え、デスクとリスク管理・モデルバリデーションの三者が同じ土俵でリスクを議論できる状態を作ります。
# %% [markdown]
# ## 理論
#
# ### 1. 3モデルの比較軸
...
# #### 数値例
#
# **数値例**：Vasicek で $a=0.3,\ b=0.02,\ \sigma=0.01,\ r_0=0.01$、$T=5$ のとき、$r(T)$ の平均は $1.78\%$・標準偏差 $1.26\%$ なので、負金利確率は $\Pr[r(T)<0]=N(-1.78/1.26)=7.90\%$ です。CIR は非負のため $0\%$ です。
# %%
T_dist = 5.0
