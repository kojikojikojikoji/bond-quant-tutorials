...
# %% [markdown]
# **数値例**：$\sigma=1.5\%$ の Hull-White ツリーで評価すると、ストレート債 $P_{\text{straight}}=105.23$、コールオプション $V_{\text{call}}=5.96$ なので $P_{\text{callable}}=105.23-5.96=99.27$ とストレート債より安くなります。
#
# **数値例**：市場価格をフェアより $1.0$ 割安と仮定すると Zスプレッド $=84.4$ bp、OAS $=18.7$ bp となり、オプションコスト $=\text{Z}-\text{OAS}=84.4-18.7=65.7$ bp です。
#
# **数値例**：$\pm100$bp 平行移動で $P_0=99.24,\ P_{-\Delta y}=104.06,\ P_{+\Delta y}=93.99,\ \Delta y=0.01$ のとき、実効デュレーション $D_{\text{eff}}=\dfrac{104.06-93.99}{2\times99.24\times0.01}=5.07$、実効コンベクシティ $C_{\text{eff}}=\dfrac{93.99+104.06-2\times99.24}{99.24\times0.01^2}=-43.6$ と負になります（負のコンベクシティ）。
# %% [markdown]
# ## スクラッチ実装
#
