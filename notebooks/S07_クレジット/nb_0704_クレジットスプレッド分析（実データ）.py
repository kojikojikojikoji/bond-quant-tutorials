...
# %% [markdown]
# **数値例**：社債利回り $y_{\text{corp}}=3.50\%$、同年限の国債利回り $2.30\%$、補間スワップレート $2.40\%$ なら、$\text{G-spread}=350-230=120$bp、$\text{I-spread}=350-240=110$bp です。差 $120-110=10$bp は、スワップスプレッド（スワップ−国債）$240-230=10$bp にちょうど一致します。
#
# **数値例**：Zスプレッドは $P=\sum_k c_k\,e^{-(z(t_k)+Z)\,t_k}$ を市場価格に合わせる定数 $Z$ です。たとえばゼロカーブ全体に一律 120bp を足して割り引いた現在価値が観測価格に一致すれば $Z=120$bp で、オプションの無い債券では OAS も同じ 120bp になります。
# %% [markdown]
# ## スクラッチ実装
#
