...
# ## 実務での位置づけ（ファンドはどう稼ぐか）
#
# CVA は、XVA デスクの本業そのものです。docs の枠組みでいうと、XVA デスクは銀行がOTCデリバティブで抱えるカウンターパーティリスクを価格化しヘッジする専門部隊です。トレーダーが顧客とスワップを約定すると、その無リスク価値からCVA（相手の破綻で被る期待損失の現在価値）を差し引いた額が「本当の」取引価値になります。XVA デスクはこのCVAをデスクから買い取り（内部で課金し）、集中管理してヘッジします。CVA の大きさは $(1-R)\int_0^T \mathbb{E}[D(t)V(t)^+]\,\mathrm{d}(1-S(t))$ で決まり、エクスポージャープロファイル（S5 の金利モンテカルロで生成する将来価値パス）と、カウンターパーティのハザードカーブ（S7-2 で CDS からブートストラップした生存確率）の掛け算です。つまりこの notebook は S5 と S7 を結合した総合演習であると同時に、XVA デスクの計算そのものの縮図です。
#
# ヘッジの対象は二系統に分かれます。第一に**市場リスク**：CVA はエクスポージャーを通じて金利に感応するため、金利デルタ・ベガを持ちます。CVA はエクスポージャー $V(t)^+$ の期待現在価値、すなわち一連のスワップションのポートフォリオとして表現でき、金利が動けば CVA も動きます。これをスワップやスワップションでヘッジします。第二に**信用リスク**：カウンターパーティのスプレッドが拡大すると生存確率 $S(t)$ が下がり CVA が増える（信用デルタ）ため、その発行体の CDS を買ってヘッジします（S7-2 の CS01 がここで効きます）。CDS が流動的でない相手は、指数 CDS やプロキシでヘッジせざるを得ず、ベーシスリスクが残ります。CVA の変動は会計上の損益に直撃するため、このヘッジ品質が XVA デスクの成績を左右します。
#
# ネッティングと担保は、CVA を削る二大レバーです。ネッティング契約下では同一カウンターパーティとの全取引を合算した純額 $V(t)^+$ でエクスポージャーを測るため、逆向きのポジションが相殺し合ってエクスポージャーが縮みます。担保（CSA）を差し入れ合えば、時価変動のたびに証拠金をやり取りしてエクスポージャーをしきい値以下に抑えられます。これらのモデル化はフル XVA（FVA/KVA 等）へ広がりますが、本 notebook はオプション回として「CVA とエクスポージャープロファイルとは何か」に答えられる水準に絞ります。銀行の XVA・カウンターパーティ信用リスク部門を志望するなら、ここが入口になります。
# %% [markdown]
# ## 理論
#
