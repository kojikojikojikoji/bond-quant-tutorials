...
# %% [markdown]
# **数値例**：ある銘柄の当日残差 $r_{i,t}=-8$ bp、時系列平均 $\bar r_i=-3$ bp、標準偏差 $s_i=2$ bp なら $z_{i,t}=(-8-(-3))/2=-2.5$ となり、平常時より 2.5 シグマぶん rich（割高）と読めます。
#

# %% [markdown]
# **数値例**：$\phi=0.85$（日次）を代入すると $h_{1/2}=-\ln 2/\ln 0.85 \approx 4.27$ 日となり、残差の乖離は約4営業日で半分に縮みます。連続時間の $\kappa=-\ln 0.85\approx 0.163$ に対応し、$h_{1/2}=\ln 2/\kappa$ も同じく約 4.27 になります。
#
