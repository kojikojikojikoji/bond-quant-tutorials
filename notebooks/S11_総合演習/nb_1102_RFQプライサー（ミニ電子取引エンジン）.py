...
# %% [markdown]
# **数値例**：モデル利回り $\hat{y}(\tau_i)=1.20\%$ の10年債に、恒常的な残差 $\bar{r}_i=+8$ bp（構造的に cheap）を足し戻すと、ミッド利回りは $y^{\text{mid}}_i = 1.20\% + 0.08\% = 1.28\%$ になります。カーブ値より 8 bp 高い（＝価格が安い）水準で値付けします。
# %% [markdown]
# **数値例**：$\delta_0=0.02$、$\gamma=0.15$、価格ボラ $\sigma^P=0.5$ 点、$T=5$ 日とすると、標準サイズ $Q=1$ では $\delta = 0.02 + \tfrac{1}{2}\cdot0.15\cdot0.5^2\cdot5\cdot1 = 0.1138$ 点、10倍サイズ $Q=10$ では $\delta = 0.9575$ 点です。在庫リスク項がサイズに線形で効き、半スプレッドは約 8.4 倍に広がります。
# %% [markdown]
# **数値例**：ミッド $m=101.20$ 点の銘柄でロング在庫 $q=+4$、$\gamma=0.15$、$\sigma^P=0.5$ 点、$T-t=5$ 日とすると、リザベーション価格は $r = 101.20 - 4\cdot0.15\cdot0.5^2\cdot5 = 101.20 - 0.75 = 100.45$ 点です。ロング在庫の分だけミッドより 0.75 点低く、bid/ask がそろって下がり在庫を減らす向きへスキューします。
# %% [markdown]
# ## スクラッチ実装
#
